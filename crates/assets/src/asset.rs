...
pub mod v1 {
    ...
    pub const MINIMUM_ASSET_ID_LENGTH: usize =
        MINIMUM_ASSET_NAMESPACE_LENGTH + 1 + MINIMUM_ASSET_REFERENCE_LENGTH;
    /// The maximum length, including separator symbols, an asset ID can have
    /// according to the minimum values defined by the CAIP-19 definition.
    pub const MAXIMUM_ASSET_ID_LENGTH: usize = MAXIMUM_NAMESPACE_LENGTH
        + 1
        + MAXIMUM_ASSET_REFERENCE_LENGTH
        + 1
        + MAXIMUM_ASSET_IDENTIFIER_LENGTH;
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            match (encoded_namespace, encoded_reference, encoded_identifier) {
                ...
                (Some(ERC20_NAMESPACE), Some(erc20_reference), None) => {
                    EvmSmartContractFungibleReference::from_utf8_encoded(erc20_reference)
                        .map(Self::Erc20)
                }
                ...
                (Some(ERC721_NAMESPACE), Some(erc721_reference), erc721_identifier) => {
                    ...
                    Ok(Self::Erc721(EvmSmartContractNonFungibleReference(reference, identifier)))
                }
                ...
                (Some(ERC1155_NAMESPACE), Some(erc1155_reference), erc1155_identifier) => {
                    let reference =
                        EvmSmartContractFungibleReference::from_utf8_encoded(erc1155_reference)?;
                    let identifier = erc1155_identifier.map_or(Ok(None), |id| {
                        EvmSmartContractNonFungibleIdentifier::from_utf8_encoded(id).map(Some)
                    })?;
                    Ok(Self::Erc1155(EvmSmartContractNonFungibleReference(reference, identifier)))
                }
    ...
    impl Display for AssetId {
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            match self {
                ...
                Self::Generic(GenericAssetId { namespace, reference, id }) => {
                    namespace.fmt(f)?;
                    write!(f, "{}", char::from(ASSET_NAMESPACE_REFERENCE_SEPARATOR))?;
                    reference.fmt(f)?;
    ...
    fn split_components(input: &[u8]) -> AssetComponents {
        let mut namespace_reference_split =
            input.splitn(2, |c| *c == ASSET_NAMESPACE_REFERENCE_SEPARATOR);
        let (parsed_namespace, reference) =
            (namespace_reference_split.next(), namespace_reference_split.next());
        ...
        let (parsed_reference, parsed_identifier) = reference.map_or((reference, None), |r| {
            let mut reference_identifier_split =
    ...
    impl Slip44Reference {
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            let decoded = str::from_utf8(input_ref).map_err(|_| {
                log::trace!(
                    "Provided input is not a valid UTF8 string as expected by a Slip44 reference."
                );
                ReferenceError::InvalidFormat
            })?;
            let parsed = U256::from_dec_str(decoded).map_err(|_| {
                log::trace!(
                    "Provided input is not a valid u256 value as expected by a Slip44 reference."
                );
                ReferenceError::InvalidFormat
            })?;
    ...
    impl GenericAssetId {
        ...
        where
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            let AssetComponents { namespace, reference, identifier } =
                split_components(input.as_ref());

            match (namespace, reference, identifier) {
                (Some(encoded_namespace), Some(encoded_reference), encoded_identifier) => {
                    Ok(Self {
                        ...
                        // successful cases.
                        id: encoded_identifier.map_or(Ok(None), |id| {
                            GenericAssetIdentifier::from_utf8_encoded(id).map(Some)
                        })?,
                    })
                }
    ...
    pub struct GenericAssetNamespace(
        pub(crate) BoundedVec<u8, ConstU32<MAXIMUM_ASSET_NAMESPACE_LENGTH_U32>>,
    );
    ...
    impl GenericAssetNamespace {
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            Ok(Self(
                Vec::<u8>::from(input_ref).try_into().map_err(|_| NamespaceError::InvalidFormat)?,
            ))
        }
    }
    ...
    impl Display for GenericAssetNamespace {
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            ...
            write!(
                ...
                "{}",
                str::from_utf8(&self.0)
                    .expect("Conversion of GenericAssetNamespace to string should never fail.")
            )
        }
    }
    ...
    pub struct GenericAssetReference(
        pub(crate) BoundedVec<u8, ConstU32<MAXIMUM_ASSET_REFERENCE_LENGTH_U32>>,
    );
    ...
    impl GenericAssetReference {
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            Ok(Self(
                Vec::<u8>::from(input_ref).try_into().map_err(|_| ReferenceError::InvalidFormat)?,
            ))
        }
    }
    ...
    impl Display for GenericAssetReference {
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            ...
            write!(
                ...
                "{}",
                str::from_utf8(&self.0)
                    .expect("Conversion of GenericAssetReference to string should never fail.")
            )
        }
    }
    ...
    pub struct GenericAssetIdentifier(
        pub(crate) BoundedVec<u8, ConstU32<MAXIMUM_ASSET_IDENTIFIER_LENGTH_U32>>,
    );
    ...
    impl Display for GenericAssetIdentifier {
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            ...
            write!(
                ...
                "{}",
                str::from_utf8(&self.0)
                    .expect("Conversion of GenericAssetIdentifier to string should never fail.")
            )
        }
    }
    ...
    mod test {
        ...
        fn test_slip44_assets() {
            ...
            for asset in valid_assets {
                let asset_id = AssetId::from_utf8_encoded(asset.as_bytes()).unwrap_or_else(|_| {
                    panic!("Asset ID {:?} should not fail to parse for slip44 assets", asset)
                });
        ...
        fn test_erc20_assets() {
            ...
            for asset in valid_assets {
                let asset_id = AssetId::from_utf8_encoded(asset.as_bytes()).unwrap_or_else(|_| {
                    panic!("Asset ID {:?} should not fail to parse for erc20 assets", asset)
                });
                // Verify that the ToString implementation prints exactly the original input
        ...
        fn test_erc721_assets() {
            ...
            for asset in valid_assets {
                let asset_id = AssetId::from_utf8_encoded(asset.as_bytes()).unwrap_or_else(|_| {
                    panic!("Asset ID {:?} should not fail to parse for erc721 assets", asset)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(asset_id.to_string(), asset.to_lowercase());
            }
        ...
        fn test_erc1155_assets() {
            ...
            for asset in valid_assets {
                let asset_id = AssetId::from_utf8_encoded(asset.as_bytes()).unwrap_or_else(|_| {
                    panic!("Asset ID {:?} should not fail to parse for erc1155 assets", asset)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(asset_id.to_string(), asset.to_lowercase());
            }
        ...
        fn test_generic_assets() {
            ...
            for asset in valid_assets {
                let asset_id = AssetId::from_utf8_encoded(asset.as_bytes()).unwrap_or_else(|_| {
                    panic!("Asset ID {:?} should not fail to parse for generic assets", asset)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(asset_id.to_string(), asset);
            }
