...
mod v1 {
    ...
    pub const MINIMUM_CHAIN_ID_LENGTH: usize =
        MINIMUM_CHAIN_NAMESPACE_LENGTH + 1 + MINIMUM_CHAIN_REFERENCE_LENGTH;
    /// The maximum length, including separator symbols, a chain ID can have
    /// according to the minimum values defined by the CAIP-2 definition.
    pub const MAXIMUM_CHAIN_ID_LENGTH: usize =
        MAXIMUM_CHAIN_NAMESPACE_LENGTH + 1 + MAXIMUM_CHAIN_REFERENCE_LENGTH;
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            match (namespace, reference) {
                ...
                (Some(DOTSAMA_NAMESPACE), Some(dotsama_reference)) => {
                    GenesisHexHash32Reference::from_utf8_encoded(dotsama_reference)
                        .map(Self::Dotsama)
                }
                ...
                (Some(SOLANA_NAMESPACE), Some(solana_reference)) => {
                    GenesisBase58Hash32Reference::from_utf8_encoded(solana_reference)
                        .map(Self::Solana)
                }
    ...
    fn split_components(input: &[u8]) -> ChainComponents {
        ...
        ChainComponents { namespace: split.next(), reference: split.next() }
    }
    ...
    impl Eip155Reference {
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            let decoded = str::from_utf8(input_ref).map_err(|_| {
                log::trace!(
                    "Provided input is not a valid UTF8 string as expected by an Eip155 reference."
                );
                ReferenceError::InvalidFormat
            })?;
            let parsed = decoded.parse::<u128>().map_err(|_| {
                log::trace!(
                    "Provided input is not a valid u128 value as expected by an Eip155 reference."
                );
                ReferenceError::InvalidFormat
            })?;
    ...
    impl GenesisHexHash32Reference {
            ...
            I: AsRef<[u8]> + Into<Vec<u8>>,
        {
            ...
            let inner: [u8; 16] = decoded.try_into().map_err(|_| {
                log::trace!(
                    "Provided input is not 16 bytes long as expected by a genesis HEX reference."
                );
                ReferenceError::InvalidFormat
            })?;
            Ok(Self(inner))
    ...
    impl GenesisBase58Hash32Reference {
        ...
        pub fn solana_mainnet() -> Self {
            ...
            Self(
                vec![
                    187, 54, 81, 91, 131, 4, 217, 218, 81, 6, 169, 34, 88, 214, 125, 109, 223, 209,
                    236, 21, 49, 109, 82,
                ]
                .try_into()
    ...
    pub struct GenericChainNamespace(
        pub(crate) BoundedVec<u8, ConstU32<MAXIMUM_CHAIN_NAMESPACE_LENGTH_U32>>,
    );
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            ...
            write!(
                ...
                str::from_utf8(&self.0)
                    .expect("Conversion of GenericChainNamespace to string should never fail.")
            )
        }
    }
    ...
    pub struct GenericChainReference(
        pub(crate) BoundedVec<u8, ConstU32<MAXIMUM_CHAIN_REFERENCE_LENGTH_U32>>,
    );
    ...
    impl Display for GenericChainReference {
        ...
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            ...
            write!(
                ...
                "{}",
                str::from_utf8(&self.0)
                    .expect("Conversion of GenericChainReference to string should never fail.")
            )
        }
    }
    ...
    mod test {
        ...
        fn test_eip155_chains() {
            let valid_chains =
                ["eip155:1", "eip155:5", "eip155:99999999999999999999999999999999", "eip155:0"];
            for chain in valid_chains {
                let chain_id = ChainId::from_utf8_encoded(chain.as_bytes()).unwrap_or_else(|_| {
                    panic!("Chain ID {:?} should not fail for eip155 chains", chain)
                });
        ...
        fn test_bip122_chains() {
            ...
            for chain in valid_chains {
                let chain_id = ChainId::from_utf8_encoded(chain.as_bytes()).unwrap_or_else(|_| {
                    panic!("Chain ID {:?} should not fail for bip122 chains", chain)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(chain_id.to_string(), chain);
            }
        ...
        fn test_dotsama_chains() {
            ...
            for chain in valid_chains {
                let chain_id = ChainId::from_utf8_encoded(chain.as_bytes()).unwrap_or_else(|_| {
                    panic!("Chain ID {:?} should not fail for dotsama chains", chain)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(chain_id.to_string(), chain);
            }
        ...
        fn test_solana_chains() {
            ...
            for chain in valid_chains {
                let chain_id = ChainId::from_utf8_encoded(chain.as_bytes()).unwrap_or_else(|_| {
                    panic!("Chain ID {:?} should not fail for solana chains", chain)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(chain_id.to_string(), chain);
            }
        ...
        fn test_generic_chains() {
            ...
            for chain in valid_chains {
                let chain_id = ChainId::from_utf8_encoded(chain.as_bytes()).unwrap_or_else(|_| {
                    panic!("Chain ID {:?} should not fail for generic chains", chain)
                });
                // Verify that the ToString implementation prints exactly the original input
                assert_eq!(chain_id.to_string(), chain);
            }
        ...
        fn test_helpers() {
            ...
            assert_eq!(ChainId::kusama().to_string(), "polkadot:b0a8d493285c2df73290dfb7e61f870f");
            assert_eq!(
