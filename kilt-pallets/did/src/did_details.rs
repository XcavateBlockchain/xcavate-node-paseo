...
use crate::{
    ...
    utils, AccountIdOf, BalanceOf, Config, DidAuthorizedCallOperationOf, DidCreationDetailsOf,
    KeyIdOf, Payload,
};
...
impl<T: Config> DidDetails<T> {
    ...
    pub fn calculate_deposit(&self, endpoint_count: u32) -> BalanceOf<T> {
        ...
        deposit = deposit.saturating_add(
            endpoint_count_as_balance.saturating_mul(T::ServiceEndpointDeposit::get()),
        );
    ...
    pub fn new_with_creation_details(
        ...
        let mut new_did_details =
            DidDetails::new(new_auth_key, current_block_number, details.clone().submitter)?;
    ...
    pub fn remove_key_agreement_key(
        ...
        key_id: KeyIdOf<T>,
            ...
            errors::StorageError::NotFound(errors::NotFoundKind::Key(
                errors::KeyType::KeyAgreement
            ))
        );
    ...
    pub fn remove_attestation_key(&mut self) -> Result<(), errors::StorageError> {
        let old_key_id = self.attestation_key.take().ok_or(errors::StorageError::NotFound(
...
pub(crate) type DidNewKeyAgreementKeySet<MaxNewKeyAgreementKeys> =
    BoundedVec<DidEncryptionKey, MaxNewKeyAgreementKeys>;
...
#[derive(Clone, RuntimeDebug, Decode, Encode, DecodeWithMemTracking, PartialEq, Eq, TypeInfo)]
