...
pub mod test {
    ...
    use crate::{
        self as did, did_details::DidVerificationKey, migrations::update_balance_for_did, mock::*,
        mock_utils::*, AccountIdOf, Config, Did, Error, HoldReason,
    };
        ...
        ExtBuilder::default()
            ...
            .build_and_execute_with_sanity_tests(None, || {
                let hold_balance_pre_migration = <<Test as Config>::Currency as InspectHold<
                    AccountIdOf<Test>,
                >>::balance_on_hold(
                    &HoldReason::Deposit.into(), &alice_did
                );
        ...
        ExtBuilder::default()
            ...
            .build_and_execute_with_sanity_tests(None, || {
                ...
                let balance_on_hold = <<Test as Config>::Currency as InspectHold<
                    AccountIdOf<Test>,
                >>::balance_on_hold(
                    &HoldReason::Deposit.into(), &alice_did.clone()
                );
