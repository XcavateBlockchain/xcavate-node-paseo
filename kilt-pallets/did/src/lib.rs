...
pub mod pallet {
    ...
    use crate::{
        did_details::{
            ...
            DidDetails, DidEncryptionKey, DidSignature, DidVerifiableIdentifier,
        ...
        T::AccountId: AsRef<[u8; 32]> + From<[u8; 32]>,
    {
        ...
        pub fn create(
            ...
            let mut did_entry =
                DidDetails::new_with_creation_details(*details, account_did_auth_key)
                    .map_err(Error::<T>::from)?;
        ...
        pub fn dispatch_as(
            ...
            #[cfg(not(feature = "runtime-benchmarks"))]
            let result =
                call.dispatch(DidRawOrigin { id: did_identifier.clone(), submitter: who }.into());
            #[cfg(feature = "runtime-benchmarks")]
        ...
        pub fn create_from_account(
            ...
            let did_entry =
                DidDetails::new(authentication_key, current_block_number, sender.clone())
                    .map_err(Error::<T>::from)?;
        ...
        T::AccountId: AsRef<[u8; 32]> + From<[u8; 32]>,
    {
        ...
        pub fn try_insert_did(
            ...
            <T as Config>::BalanceMigrationManager::exclude_key_from_migration(
                &Did::<T>::hashed_key_for(&did_identifier),
            );
        ...
        fn try_update_deposit(
            did_details: &mut DidDetails<T>,
            did_subject: &DidIdentifierOf<T>,
        ) -> DispatchResult {
            ...
            let is_key_migrated =
                <T as Config>::BalanceMigrationManager::is_key_migrated(&hashed_key);

            match new_required_deposit.cmp(&did_details.deposit.amount) {
                core::cmp::Ordering::Greater => {
                    let deposit_to_reserve =
                        new_required_deposit.saturating_sub(did_details.deposit.amount);
                    ...
                    if is_key_migrated {
                        ...
                        did_details.deposit.amount =
                            did_details.deposit.amount.saturating_add(deposit_to_reserve);
                    } else {
                ...
                core::cmp::Ordering::Less => {
                    let deposit_to_release =
                        did_details.deposit.amount.saturating_sub(new_required_deposit);
                    if is_key_migrated {
                    ...
                    did_details.deposit.amount =
                        did_details.deposit.amount.saturating_sub(deposit_to_release);
                }
                _ => (),
            };
            Ok(())
        ...
        pub fn verify_payload_signature_with_did_key_type(
            ...
            // an error
            verification_key.verify_signature(payload, signature).map_err(DidError::Signature)
        }
        ...
        pub fn delete_did(
            ...
            // `NotFound`.
                ...
                <<T::DidLifecycleHooks as DidLifecycleHooks<T>>::DeletionHook as DidDeletionHook<
                    T,
                >>::can_delete(&did_subject,),
            ...
            let is_key_migrated = <T as Config>::BalanceMigrationManager::is_key_migrated(
                &Did::<T>::hashed_key_for(did_subject.clone()),
            );
            if is_key_migrated {
    ...
    impl<T: Config>
        StorageDepositCollector<AccountIdOf<T>, DidIdentifierOf<T>, T::RuntimeHoldReason>
        for DidDepositCollector<T>
    {
        ...
        fn deposit(
            ...
            Deposit<AccountIdOf<T>, <Self::Currency as Inspect<AccountIdOf<T>>>::Balance>,
            DispatchError,
        > {
        ...
        fn deposit_amount(
            key: &DidIdentifierOf<T>,
        ) -> <Self::Currency as Inspect<AccountIdOf<T>>>::Balance {
