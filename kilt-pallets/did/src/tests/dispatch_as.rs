...
fn blueprint_successful_dispatch<FB: FnOnce(), FA: FnOnce()>(
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(None, || {
            before();
            let did_details_before =
                DidStorage::<Test>::get(did_identifier.clone()).expect("Did must exists");
            ...
            let did_details_after =
                DidStorage::<Test>::get(did_identifier.clone()).expect("Did must not be deleted");
