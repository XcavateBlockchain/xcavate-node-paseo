...
use crate::{
    did_details::DidVerificationKey, mock::*, mock_utils::*,
    tests::dispatch_as::blueprint_failed_dispatch, AccountIdOf, DidIdentifierOf, Error,
};
...
fn successful_key_dispatch_ed25519() {
    ...
    let verification_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_02.as_ref()));
    let attestation_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    blueprint_successful_dispatch_with_key(
        did_identifier,
        caller,
        verification_key,
        attestation_key,
...
fn successful_key_dispatch_sr25519() {
    ...
    let did_identifier = ACCOUNT_02;
    let attestation_key =
        DidVerificationKey::Sr25519(sr25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    let verification_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    blueprint_successful_dispatch_with_key(
        did_identifier,
...
fn failed_no_match_ed25519() {
    let attestation_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_01.as_ref()));
    let delegation_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    let authentication_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    blueprint_failed_dispatch_with_key(
        ACCOUNT_00,
        authentication_key,
        attestation_key,
        delegation_key,
...
fn failed_no_match_sr25519() {
    let attestation_key =
        DidVerificationKey::Sr25519(sr25519::Public::from_raw(*ACCOUNT_01.as_ref()));
    let delegation_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
    let authentication_key =
        DidVerificationKey::Ed25519(ed25519::Public::from_raw(*ACCOUNT_00.as_ref()));
