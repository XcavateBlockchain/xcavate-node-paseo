...
use crate::{
    self as did, did_details::DidVerificationKey, mock::*, mock_utils::*,
    service_endpoints::DidEndpoint, HoldReason,
};
...
fn check_deposit_change_by_adding_service_endpoint() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert!(did::ServiceEndpoints::<Test>::get(alice_did.clone(), new_service_endpoint.id)
                .is_some());

            assert!(
                did::ServiceEndpoints::<Test>::get(alice_did, new_service_endpoint2.id).is_none()
            );
        });
}
...
fn check_service_addition_no_prior_service_successful() {
    ...
    let old_did_details = generate_base_did_details::<Test>(
        DidVerificationKey::from(auth_key.public()),
        Some(alice_did.clone()),
    );
...
fn check_max_service_id_length_addition_error() {
    ...
    let old_did_details =
        generate_base_did_details::<Test>(DidVerificationKey::from(auth_key.public()), None);

    let origin = build_test_origin(alice_did.clone(), alice_did.clone());

    ExtBuilder::default().with_dids(vec![(alice_did, old_did_details)]).build(None).execute_with(
        || {
            assert_noop!(
                Did::add_service_endpoint(origin, new_service_endpoint),
