...
fn check_successful_authentication_key_update() {
    ...
    let old_did_details = generate_base_did_details::<Test>(
        DidVerificationKey::from(old_auth_key.public()),
...
fn check_reused_key_authentication_key_update() {
    ...
    let mut old_did_details = generate_base_did_details::<Test>(
        DidVerificationKey::from(old_auth_key.public()),
        Some(alice_did.clone()),
    );
...
fn check_successful_key_agreement_key_deletion() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(None, || {
            assert_ok!(
                Did::remove_key_agreement_key(origin, generate_key_id(&old_enc_key.into()),)
            );
            let new_did_details =
                Did::get_did(&alice_did).expect("ALICE_DID should be present on chain.");
