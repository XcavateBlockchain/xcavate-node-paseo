...
use crate::{
    default_weights::WeightInfo, Config, DelegationHierarchies, DelegationNodeIdOf,
    DelegationNodes, DelegatorIdOf, Error, Pallet, Permissions,
};
...
#[derive(
    Clone, Debug, Encode, Decode, DecodeWithMemTracking, PartialEq, Eq, TypeInfo, MaxEncodedLen,
)]
...
impl<T: Config>
    attestation::AttestationAccessControl<
        DelegatorIdOf<T>,
    ...
    > for DelegationAc<T>
{
    fn can_attest(
        ...
        let delegation_node = DelegationNodes::<T>::get(self.authorization_id())
            .ok_or(Error::<T>::DelegationNotFound)?;
        let root = DelegationHierarchies::<T>::get(delegation_node.hierarchy_root_id)
            .ok_or(Error::<T>::DelegationNotFound)?;
        ensure!(
            // has permission
...
mod tests {
    ...
    fn test_can_attest() {
        ...
        let parent_node = DelegationNode {
            ...
            deposit: Deposit { owner: ACCOUNT_00, amount: <Test as Config>::Deposit::get() },
        };
        let claim_hash = claim_hash_from_seed(CLAIM_HASH_SEED_01);
        let ac_info = Some(DelegationAc { subject_node_id: parent_id, max_checks: 1 });
        ...
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                let stored_attestation = Attestation::attestations(claim_hash)
                    .expect("Attestation should be present on chain.");
