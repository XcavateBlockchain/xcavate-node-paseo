...
pub use crate::{
    access_control::DelegationAc, default_weights::WeightInfo, delegation_hierarchy::*, pallet::*,
};
...
pub mod pallet {
    ...
    /// information.
    pub type DelegateSignatureTypeOf<T> =
        <DelegationSignatureVerificationOf<T> as VerifySignature>::Signature;
    ...
    pub type DelegationNodes<T> =
        StorageMap<_, Blake2_128Concat, DelegationNodeIdOf<T>, DelegationNodeOf<T>>;
    ...
    pub type DelegationHierarchies<T> = StorageMap<
        _,
        Blake2_128Concat,
        DelegationNodeIdOf<T>,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add_delegation(
            ...
            let parent_node =
                <DelegationNodes<T>>::get(parent_id).ok_or(Error::<T>::ParentDelegationNotFound)?;
            let hierarchy_root_id = parent_node.hierarchy_root_id;

            // Calculate the hash root
            let hash_root = Self::calculate_delegation_creation_hash(
                &delegation_id,
                &hierarchy_root_id,
                &parent_id,
                &permissions,
            ...
            DelegationSignatureVerificationOf::<T>::verify(
                &delegate,
                &hash_root.encode(),
                &delegate_signature,
            )
            ...
            Self::store_delegation_under_parent(
                ...
                DelegationNode::new_node(
                    hierarchy_root_id,
                    parent_id,
                    DelegationDetails { owner: delegate.clone(), permissions, revoked: false },
                    payer.clone(),
        ...
        pub fn revoke_delegation(
            ...
            let (revocation_checks, _) =
                Self::revoke(&delegation_id, &invoker, max_revocations.saturating_add(1))?;
        ...
        pub fn remove_delegation(
            ...
            let delegation =
                DelegationNodes::<T>::get(delegation_id).ok_or(Error::<T>::DelegationNotFound)?;
        ...
        pub fn update_deposit(
            ...
            DelegationDepositCollector::<T>::update_deposit::<BalanceMigrationManagerOf<T>>(
                &delegation_id,
            )?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(crate) fn create_and_store_new_hierarchy(
            ...
            DelegationDepositCollector::<T>::create_deposit(
                deposit_owner.clone(),
                <T as Config>::Deposit::get(),
            )?;
            <T as Config>::BalanceMigrationManager::exclude_key_from_migration(
                &DelegationNodes::<T>::hashed_key_for(root_id),
            );
        ...
        pub(crate) fn store_delegation_under_parent(
            ...
            <T as Config>::BalanceMigrationManager::exclude_key_from_migration(
                &DelegationNodes::<T>::hashed_key_for(delegation_id),
            );
        ...
        pub fn is_delegating(
            ...
            let delegation_node =
                <DelegationNodes<T>>::get(delegation).ok_or(Error::<T>::DelegationNotFound)?;
    ...
    impl<T: Config>
        StorageDepositCollector<AccountIdOf<T>, DelegationNodeIdOf<T>, T::RuntimeHoldReason>
        for DelegationDepositCollector<T>
    {
        ...
        ) -> Result<sp_std::vec::Vec<u8>, DispatchError> {
        ...
        > {
            let delegation_node =
                DelegationNodes::<T>::get(key).ok_or(Error::<T>::DelegationNotFound)?;
            Ok(delegation_node.deposit)
        }

        fn deposit_amount(
            _key: &DelegationNodeIdOf<T>,
        ...
        fn store_deposit(
            ...
            let delegation_node =
                DelegationNodes::<T>::get(key).ok_or(Error::<T>::DelegationNotFound)?;
            DelegationNodes::<T>::insert(key, DelegationNode { deposit, ..delegation_node });
