...
fn is_delegating_direct_revoked() {
    ...
    let delegation_id = delegation_id_from_seed::<Test>(DELEGATION_ID_SEED_2);
    let mut delegation_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        user_3.clone(),
        Some(parent_id),
        ACCOUNT_00,
...
fn calculate_reference_root_hash() {
    let delegation_id = sp_core::H256::from_slice(&[
        185, 126, 188, 188, 245, 142, 61, 132, 79, 33, 135, 134, 152, 6, 203, 109, 120, 38, 111,
        103, 61, 241, 239, 138, 46, 66, 215, 123, 126, 94, 77, 66,
    ]);

    let hierarchy_root_id = sp_core::H256::from_slice(&[
        70, 167, 51, 210, 165, 28, 28, 139, 12, 153, 171, 25, 102, 201, 169, 88, 65, 125, 169, 191,
        42, 153, 90, 138, 192, 111, 144, 8, 231, 196, 167, 51,
    ]);

    let parent_id = sp_core::H256::from_slice(&[
        41, 25, 184, 103, 76, 11, 115, 50, 39, 65, 32, 13, 205, 136, 227, 114, 253, 200, 50, 199,
        71, 206, 216, 234, 99, 37, 221, 21, 199, 111, 91, 209,
    ]);
    ...
    assert_eq!(
        ...
        sp_core::H256::from_slice(&[
            163, 68, 221, 218, 225, 105, 180, 154, 248, 52, 210, 46, 111, 20, 142, 1, 154, 18, 189,
            126, 217, 41, 151, 135, 19, 250, 243, 130, 33, 174, 133, 4,
        ])
    )
}
