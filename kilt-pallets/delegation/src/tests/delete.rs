...
fn tree_hierarchy_revoke_and_remove_root_successful() {
    ...
    let delegation1_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        revoker.clone(),
        Some(hierarchy_root_id),
        ACCOUNT_00,
    ...
    let delegation2_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        delegate,
        Some(hierarchy_root_id),
        ACCOUNT_01,
...
fn exact_children_max_revocations_revoke_and_remove_root_error() {
    ...
    let delegation3_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        delegate,
        Some(delegation1_id),
        ACCOUNT_01,
...
fn remove_children_gas_runs_out() {
    ...
    let delegation4_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        child,
        Some(delegation3_id),
        ACCOUNT_02,
