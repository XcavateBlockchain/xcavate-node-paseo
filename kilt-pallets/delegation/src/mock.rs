...
use crate::{
    self as delegation, AccountIdOf, Config, CurrencyOf, DelegationDetails,
    DelegationHierarchyDetails, DelegationNode, DelegationNodeOf, DelegatorIdOf, Permissions,
};
...
where
    ...
    T::Hash: From<H256>,
{
    DelegationHierarchyDetails { ctype_hash: ctype_mock::get_ctype_hash::<T>(true) }
}
...
pub fn generate_base_delegation_details<T: Config>(
    ...
    DelegationDetails { owner, permissions: Permissions::DELEGATE, revoked: false }
}
...
where
    ...
    T::Hash: From<H256>,
{
    DelegationHierarchyCreationOperation { id, ctype_hash: ctype::mock::get_ctype_hash::<T>(true) }
}

#[cfg(test)]
pub(crate) mod runtime {
    use crate::{
        BalanceOf, DelegateSignatureTypeOf, DelegationAc, DelegationNodeIdOf, DelegationNodeOf,
    };
    ...
    pub(crate) fn generate_base_delegation_deposit_claim_operation(
        ...
        DelegationDepositClaimOperation { delegation_id, max_removals: 0u32 }
    }
        ...
        pub fn build_and_execute_with_sanity_tests(self, test: impl FnOnce()) {
            self.build().execute_with(|| {
                ...
                crate::try_state::do_try_state::<Test>()
                    .expect("Sanity test for delegation failed.");
            })
        }
