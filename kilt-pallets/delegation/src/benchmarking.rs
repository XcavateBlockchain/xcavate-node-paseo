...
where
    ...
    <T as Config>::EnsureOrigin:
        GenerateBenchmarkOrigin<T::RuntimeOrigin, T::AccountId, T::DelegationEntityId>,
    ...
    <T as Config>::Currency: Mutate<T::AccountId>,
{
    ...
    for c in 0..children_per_level.get() {
        ...
        let hash: Vec<u8> = Pallet::<T>::calculate_delegation_creation_hash(
            &delegation_id,
            &root_id,
            &parent_id,
            &permissions,
        ...
        first_leaf =
            first_leaf.or_else(|| Some((delegation_acc_public, delegation_acc_id, delegation_id)));
    }
...
pub fn setup_delegations<T>(
    ...
    (sr25519::Public, T::DelegationNodeId, sr25519::Public, T::DelegationNodeId),
...
{
    let (
        DelegationTriplet::<T> { public: root_public, acc: root_acc, delegation_id: hierarchy_id },
        _,
