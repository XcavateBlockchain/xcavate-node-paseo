...
use did_lookup::linkable_account::LinkableAccountId;
...
benchmarks! {
    where_clause {
        ...
        <T as did_lookup::Config>::Currency: ReservableCurrency<AccountIdOf<T>,Balance = <<T as did_lookup::Config>::Currency as Inspect<AccountIdOf<T>>>::Balance>,
        ...
        <T as pallet_balances::Config>::RuntimeHoldReason: From<delegation::HoldReason> + From<did_lookup::HoldReason> + From<pallet_web3_names::HoldReason> + From<public_credentials::HoldReason>,
    }
    ...
    did_lookup_migration_weight {
        ...
        let did: <T as did_lookup::Config>::DidIdentifier = account("did", 0, SEED);

        pallet_balances::Pallet::<T>::set_balance(&sender, KILT.saturated_into());
        did_lookup::Pallet::<T>::add_association(sender.clone(), did, linkable_id.clone()).expect("should create association");
        kilt_support::migration::translate_holds_to_reserve::<T>(did_lookup::HoldReason::Deposit.into());
