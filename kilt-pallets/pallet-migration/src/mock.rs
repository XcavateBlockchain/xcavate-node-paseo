...
use sp_core::{ed25519, ConstBool, ConstU128, ConstU16, ConstU32, ConstU64};
use sp_runtime::{
...
parameter_types! {
    #[derive(Clone, TypeInfo, Debug, PartialEq, Eq, Encode, Decode, DecodeWithMemTracking)]
...
impl TryFrom<Vec<u8>> for TestSubjectId {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        let inner: [u8; 32] =
            value.try_into().map_err(|_| public_credentials::Error::<Test>::InvalidInput)?;
        if inner[0] == 255 {
...
pub(crate) const LINKABLE_ACCOUNT_00: LinkableAccountId =
    LinkableAccountId::AccountId32(ACCOUNT_00);
...
pub(crate) struct ExtBuilder {
    ...
    connections:
        Vec<(AccountId, SubjectId, pallet_did_lookup::linkable_account::LinkableAccountId)>,
...
impl ExtBuilder {
    ...
    pub fn with_dids(
        mut self,
        dids: Vec<(DidIdentifier, did::did_details::DidDetails<Test>)>,
    ) -> Self {
    ...
    pub fn with_attestations(
        mut self,
        attestations: Vec<(
            attestation::ClaimHashOf<Test>,
            attestation::AttestationDetailsOf<Test>,
        )>,
    ) -> Self {
    ...
    pub fn with_delegations(
        mut self,
        delegations: Vec<(
            delegation::DelegationNodeIdOf<Test>,
            delegation::DelegationNodeOf<Test>,
        )>,
    ) -> Self {
    ...
    pub(crate) fn build(self) -> sp_io::TestExternalities {
        ...
        ext.execute_with(|| {
            ...
            delegation::mock::initialize_pallet::<Test>(
                self.delegations,
                self.delegation_hierarchies,
            );
