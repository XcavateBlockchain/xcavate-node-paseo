...
pub mod pallet {
    ...
    #[derive(
        Encode, Decode, DecodeWithMemTracking, TypeInfo, Debug, Clone, PartialEq, Eq, DefaultNoBound,
    )]
    pub struct EntriesToMigrate<T>
    where
        T: Config + did::Config + delegation::Config + public_credentials::Config,
    {
        ...
        pub public_credentials:
            BoundedVec<(SubjectIdOf<T>, CredentialIdOf<T>), <T as Config>::MaxMigrationsPerPallet>,
    }
    ...
    impl<T: Config> BalanceMigrationManager<AccountIdOf<T>, BalanceOf<T>> for Pallet<T> {
        ...
        fn release_reserved_deposit(user: &AccountIdOf<T>, balance: &BalanceOf<T>) {
            <<T as Config>::Currency as ReservableCurrency<AccountIdOf<T>>>::unreserve(
                user, *balance,
            );
        }
    }
