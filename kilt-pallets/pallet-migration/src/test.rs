...
use delegation::{
    mock::{
        delegation_id_from_seed, generate_base_delegation_hierarchy_details,
        generate_base_delegation_node, get_delegation_hierarchy_id,
    },
    DelegationNodes,
};
use did::{
...
use public_credentials::{
    mock::{
        generate_base_credential_entry, generate_base_public_credential_creation_op,
...
fn check_succesful_migration() {
    ...
    let mut parent_node = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        creator.clone(),
    ...
    let mut did_details = generate_base_did_details::<Test>(
        DidVerificationKey::from(auth_key.public()),
        Some(ACCOUNT_00),
    );
    did_details.deposit.amount = MICRO_KILT;

    // public credentials
    let deposit = kilt_support::Deposit { owner: ACCOUNT_00, amount: MICRO_KILT };
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let attestation = BoundedVec::try_from([claim_hash].to_vec())
                .expect("Vec init should not fail for attestaions");

            let delegation = BoundedVec::try_from([parent_id].to_vec())
                .expect("Vec init should not fail for delegations");

            let did = BoundedVec::try_from([alice_did].to_vec())
                .expect("Vec init should not fail for did");

            let lookup = BoundedVec::try_from([LINKABLE_ACCOUNT_00].to_vec())
                .expect("Vec init should not fail for lookup");
            ...
            let requested_migrations =
                EntriesToMigrate { attestation, delegation, did, lookup, public_credentials };
            ...
            assert_eq!(count_migrated_entries_post_migration, 5);
            ...
            let attestaion_deposit = pallet_balances::Pallet::<Test>::balance_on_hold(
                &attestation::HoldReason::Deposit.into(),
                &ACCOUNT_00,
            );

            let did_deposit = pallet_balances::Pallet::<Test>::balance_on_hold(
                &did::HoldReason::Deposit.into(),
                &ACCOUNT_00,
            );

            let delegation_deposit = pallet_balances::Pallet::<Test>::balance_on_hold(
...
fn check_attempt_to_migrate_already_migrated_keys() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // to the storage.
            assert_storage_noop!(Migration::update_balance(
                RuntimeOrigin::signed(ACCOUNT_00),
                requested_migrations
            )
...
fn check_excluded_keys_attestation() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let requested_migrations = EntriesToMigrate { attestation, ..Default::default() };
...
fn check_excluded_keys_delegation() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let delegation = BoundedVec::try_from([delegation_id].to_vec())
                .expect("Vec init should not fail for attestaions");

            let requested_migrations = EntriesToMigrate { delegation, ..Default::default() };
...
fn check_excluded_keys_did() {
    ...
    ExtBuilder::default().with_balances(vec![(ACCOUNT_00, KILT)]).build().execute_with(|| {
        ...
        let requested_migrations = EntriesToMigrate { did, ..Default::default() };
...
fn check_excluded_keys_lookup() {
    ...
    let sig_alice_0 = MultiSignature::from(pair_alice.sign(
        &[b"<Bytes>", get_challenge(&DID_00, expire_at).as_bytes(), b"</Bytes>"].concat()[..],
    ));
    ...
    ExtBuilder::default().with_balances(vec![(ACCOUNT_00, KILT)]).build().execute_with(|| {
        ...
        let lookup =
            BoundedVec::try_from([linked_acc].to_vec()).expect("Vec init should not fail for did");

        let requested_migrations = EntriesToMigrate { lookup, ..Default::default() };
...
fn check_excluded_keys_public_credentials() {
    ...
    let credential_id: CredentialIdOf<Test> =
        generate_credential_id::<Test>(&new_credential, &attester);
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let requested_migrations =
                EntriesToMigrate { public_credentials, ..Default::default() };
...
fn migrate_key_by_update_deposit_did() {
    ...
    let mut details = generate_base_did_details::<Test>(
        DidVerificationKey::from(auth_key.public()),
        Some(ACCOUNT_00),
    );
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(Did::update_deposit(RuntimeOrigin::signed(ACCOUNT_00), alice_did.clone()));
...
fn migrate_key_by_update_deposit_delegation() {
    ...
    let delegation_details = generate_base_delegation_node::<Test>(
        hierarchy_root_id,
        creator.clone(),
        Some(hierarchy_root_id),
        ACCOUNT_00,
...
fn migrate_key_by_update_deposit_public_credentials() {
    ...
    let deposit = kilt_support::Deposit { owner: ACCOUNT_00, amount: 10 * MICRO_KILT };
...
fn migrate_key_by_update_deposit_attestation() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(Attestation::update_deposit(RuntimeOrigin::signed(ACCOUNT_00), claim_hash));
