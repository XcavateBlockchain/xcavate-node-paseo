...
use frame_support::{
    assert_noop, assert_ok, crypto::ecdsa::ECDSAExt, traits::fungible::InspectHold,
};
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            assert_eq!(
                ...
                Some(ConnectionRecord {
                    did: DID_00,
                    deposit: Deposit { owner: ACCOUNT_00, amount: 10 }
                })
            );
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            let sig_alice_0 = MultiSignature::from(
                pair_alice.sign(
                    &[b"<Bytes>", get_challenge(&DID_00, expire_at).as_bytes(), b"</Bytes>"]
                        .concat()[..],
                ),
            );
            let sig_alice_1 = MultiSignature::from(
                pair_alice.sign(
                    &[b"<Bytes>", get_challenge(&DID_01, expire_at).as_bytes(), b"</Bytes>"]
                        .concat()[..],
                ),
            );
            ...
            assert!(ConnectedAccounts::<Test>::get(
                DID_00,
                LinkableAccountId::from(account_hash_alice.clone())
            )
            .is_some());
            assert_eq!(
                Balances::balance_on_hold(&HoldReason::Deposit.into(), &ACCOUNT_00),
            ...
            assert_eq!(
                ...
                Some(ConnectionRecord {
                    did: DID_01,
                    deposit: Deposit { owner: ACCOUNT_01, amount: 10 }
                })
            );
            assert!(ConnectedAccounts::<Test>::get(
                DID_00,
            ...
            assert!(ConnectedAccounts::<Test>::get(
                DID_01,
                LinkableAccountId::from(account_hash_alice)
            )
            .is_some());
...
fn test_add_eth_association() {
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            assert!(ConnectedAccounts::<Test>::get(DID_00, LinkableAccountId::from(eth_account))
                .is_some());
            assert_eq!(
                Balances::balance_on_hold(&HoldReason::Deposit.into(), &ACCOUNT_00),
                <Test as crate::Config>::Deposit::get()
...
fn test_add_association_account_invalid_signature() {
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            let sig_alice_0 =
                MultiSignature::from(pair_alice.sign(&Encode::encode(&(&DID_01, expire_at))[..]));
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            let sig_alice_0 = MultiSignature::from(pair_alice.sign(
                &[b"<Bytes>", &Encode::encode(&(&DID_01, expire_at))[..], b"</Bytes>"].concat()[..],
            ));
...
fn test_remove_association_sender() {
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            ...
            assert!(ConnectedAccounts::<Test>::get(DID_01, LinkableAccountId::from(ACCOUNT_00))
                .is_none());
            assert_eq!(Balances::balance_on_hold(&HoldReason::Deposit.into(), &ACCOUNT_00), 0);
        });
}
