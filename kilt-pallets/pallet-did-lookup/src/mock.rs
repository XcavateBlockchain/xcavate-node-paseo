...
use crate::{
    self as pallet_did_lookup, linkable_account::LinkableAccountId, AccountIdOf, BalanceOf, Config,
    ConnectedAccounts, ConnectedDids, ConnectionRecord, DidIdentifierOf,
    LinkableAccountDepositCollector,
};
...
pub(crate) const LINKABLE_ACCOUNT_01: LinkableAccountId =
    LinkableAccountId::AccountId32(ACCOUNT_01);
...
impl ExtBuilder {
    ...
    pub fn build_and_execute_with_sanity_tests(self, test: impl FnOnce()) {
        self.build().execute_with(|| {
            test();
            crate::try_state::do_try_state::<Test, _>()
                .expect("Sanity test for did lookup failed.");
        })
    }
