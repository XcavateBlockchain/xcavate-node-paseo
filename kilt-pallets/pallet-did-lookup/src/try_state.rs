...
pub(crate) fn do_try_state<T: Config<I>, I: 'static>() -> Result<(), TryRuntimeError> {
    ...
    ConnectedDids::<T, I>::iter().try_for_each(
        |(account, record)| -> Result<(), TryRuntimeError> {
            ensure!(
                ConnectedAccounts::<T, I>::contains_key(&record.did, &account),
                log_and_return_error_message(format!(
                    "Account {:?} with did {:?} not found",
                    record.did, account
                ))
            );
            Ok(())
    ...
    ConnectedAccounts::<T, I>::iter().try_for_each(
        |(did_identifier, linked_account_id, _)| -> Result<(), TryRuntimeError> {
                ...
                ConnectedDids::<T, I>::get(&linked_account_id).expect("Unknown did").did
                    == did_identifier,
                log_and_return_error_message(format!(
    ...
    if <T as Config<I>>::UniqueLinkingEnabled::get() {
        let mut did_linked_to_accounts =
            ConnectedDids::<T, I>::iter_values().map(|ConnectionRecord { did, .. }| did);

        did_linked_to_accounts.try_for_each(|did_identifier| -> Result<(), TryRuntimeError> {
            let linked_accounts =
                ConnectedAccounts::<T, I>::iter_key_prefix(&did_identifier).count();
            ensure!(
