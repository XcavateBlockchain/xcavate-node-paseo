...
#[derive(
    Clone, Debug, Eq, PartialEq, Encode, Decode, DecodeWithMemTracking, MaxEncodedLen, TypeInfo,
)]
...
impl AssociateAccountRequest {
    pub fn verify<DidIdentifier: AsRef<[u8]>, BlockNumber: Debug>(
        ...
        match self {
            AssociateAccountRequest::Polkadot(acc, proof) => {
                proof.verify(
                    &get_wrapped_payload(
                        &encoded_payload[..],
    ...
    pub fn get_linkable_account(&self) -> LinkableAccountId {
        match self {
            AssociateAccountRequest::Polkadot(acc, _) => {
                LinkableAccountId::AccountId32(acc.clone())
            }
