...
pub use crate::{
    access_control::AttestationAccessControl, attestations::AttestationDetails,
...
pub mod pallet {
    ...
    pub type AttestationDetailsOf<T> = AttestationDetails<
        CtypeHashOf<T>,
        AttesterOf<T>,
        AuthorizationIdOf<T>,
        AccountIdOf<T>,
    ...
    pub type Attestations<T> =
        StorageMap<_, Blake2_128Concat, ClaimHashOf<T>, AttestationDetailsOf<T>>;
    ...
    pub type ExternalAttestations<T> = StorageDoubleMap<
        _,
        Twox64Concat,
        AuthorizationIdOf<T>,
        Blake2_128Concat,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add(
            ...
            let deposit =
                AttestationStorageDepositCollector::<T>::create_deposit(payer, deposit_amount)?;
            <T as Config>::BalanceMigrationManager::exclude_key_from_migration(
                &Attestations::<T>::hashed_key_for(claim_hash),
            );
        ...
        pub fn revoke(
            ...
            let authorized_by = if attester != who {
                let attestation_auth_id =
                    attestation.authorization_id.as_ref().ok_or(Error::<T>::NotAuthorized)?;
            ...
            Attestations::<T>::insert(
                claim_hash,
                AttestationDetails { revoked: true, ..attestation },
            );
        ...
        pub fn update_deposit(origin: OriginFor<T>, claim_hash: ClaimHashOf<T>) -> DispatchResult {
            ...
            AttestationStorageDepositCollector::<T>::update_deposit::<BalanceMigrationManagerOf<T>>(
                &claim_hash,
            )?;
