...
use crate::{
    pallet::AuthorizationIdOf, AccountIdOf, AttestationAccessControl, AttestationDetails,
...
pub fn generate_base_attestation<T>(
    ...
    T::Hash: From<H256>,
{
    AttestationDetails {
        ...
        revoked: false,
        deposit: Deposit::<AccountIdOf<T>, BalanceOf<T>> { owner: payer, amount: deposit },
    }
}
...
pub fn sr25519_did_from_public_key(public_key: &[u8; 32]) -> SubjectId {
    MultiSigner::from(sr25519::Public::from_raw(*public_key)).into_account().into()
}
...
pub(crate) mod runtime {
    ...
    pub(crate) fn events() -> Vec<Event<Test>> {
        System::events()
            .into_iter()
            .map(|r| r.event)
            .filter_map(|e| if let RuntimeEvent::Attestation(e) = e { Some(e) } else { None })
            .collect::<Vec<_>>()
    }
        ...
        pub fn build_and_execute_with_sanity_tests(self, test: impl FnOnce()) {
            self.build().execute_with(|| {
                ...
                crate::try_state::do_try_state::<Test>()
                    .expect("Sanity test for attestation failed.");
            })
        }
