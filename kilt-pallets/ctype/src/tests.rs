    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let stored_ctype_creator =
                Ctype::ctypes(ctype_hash).expect("CType hash should be present on chain.");

            // Verify the CType has the right owner and block number
            assert_eq!(
                stored_ctype_creator,
                ctype::CtypeEntryOf::<Test> { creator, created_at: 200 }
            );
            assert_eq!(
...
fn set_block_number_ok() {
    ...
    ExtBuilder::default().with_ctypes(vec![(ctype_hash, creator)]).build().execute_with(|| {
