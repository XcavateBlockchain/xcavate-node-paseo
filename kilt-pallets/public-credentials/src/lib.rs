...
pub use crate::{
    access_control::AccessControl as PublicCredentialsAccessControl, credentials::*,
    default_weights::WeightInfo, pallet::*,
};
...
pub mod pallet {
    ...
    pub type CredentialIdOf<T> =
        <<T as Config>::CredentialHash as sp_runtime::traits::Hash>::Output;
    ...
    pub type CredentialSubjects<T> =
        StorageMap<_, Blake2_128Concat, CredentialIdOf<T>, SubjectIdOf<T>>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add(
            ...
            let Credential { ctype_hash, subject, authorization, .. } = *credential.clone();
            ...
            let credential_id = T::CredentialHash::hash(
                &[&credential.encode()[..], &attester.encode()[..]].concat()[..],
            );
            ...
            let subject = T::SubjectId::try_from(subject.into_inner())
            ...
            let deposit =
                PublicCredentialDepositCollector::<T>::create_deposit(payer, deposit_amount)
                    .map_err(|_| Error::<T>::UnableToPayFees)?;

            <T as Config>::BalanceMigrationManager::exclude_key_from_migration(
                &Credentials::<T>::hashed_key_for(&subject, &credential_id),
            );
            ...
            Self::deposit_event(Event::CredentialStored { subject_id: subject, credential_id });
        ...
        pub fn remove(
            ...
            let (credential_subject, credential_entry) =
                Self::retrieve_credential_entry(&credential_id)?;

            let ac_weight_used = if credential_entry.attester == caller {
                Weight::zero()
            } else {
                let credential_auth_id =
                    credential_entry.authorization_id.as_ref().ok_or(Error::<T>::NotAuthorized)?;
                authorization
        ...
        pub fn update_deposit(
            ...
            PublicCredentialDepositCollector::<T>::update_deposit::<BalanceMigrationManagerOf<T>>(
                &credential_id,
            )?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn remove_credential_entry(
            ...
            let details = Credentials::<T>::take(&credential_subject, &credential_id)
            ...
            Credentials::<T>::try_mutate(credential_subject, credential_id, |credential_entry| {
                if let Some(credential) = credential_entry {
                    ...
                    let additional_weight = if *caller == credential.attester {
                        ...
                        let credential_auth_id = credential
                            .authorization_id
                            .as_ref()
                            .ok_or(Error::<T>::NotAuthorized)?;
                        authorization
