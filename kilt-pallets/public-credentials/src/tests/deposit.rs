...
use crate::{
    mock::*, Config, CredentialIdOf, CredentialSubjects, Credentials, Error, Event, HoldReason,
};
...
fn reclaim_deposit_successful() {
    ...
    let new_credential =
        generate_base_credential_entry::<Test>(ACCOUNT_00, 0, attester, None, None);
...
fn test_change_deposit_owner() {
    ...
    let new_credential = generate_base_credential_entry::<Test>(
        ...
        attester.clone(),
        Some(ctype_hash_1),
        Some(Deposit { owner: ACCOUNT_00, amount: deposit }),
    );
    let credential_id: CredentialIdOf<Test> = CredentialIdOf::<Test>::default();
...
fn test_change_deposit_owner_unauthorized() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute_with_sanity_tests(|| {
            assert_noop!(
                PublicCredentials::change_deposit_owner(
                    DoubleOrigin(ACCOUNT_01, evil.clone()).into(),
                    credential_id
                ),
                Error::<Test>::NotAuthorized
            );
...
fn test_update_deposit() {
    ...
    let new_credential = generate_base_credential_entry::<Test>(
        ...
        attester.clone(),
        Some(ctype_hash_1),
        Some(Deposit { owner: ACCOUNT_00, amount: deposit_old }),
    );
    let credential_id: CredentialIdOf<Test> = CredentialIdOf::<Test>::default();
