...
use crate::{
    AccountIdOf, Config, CredentialIdOf, Credentials, CurrencyOf, Error, HoldReason, SubjectIdOf,
};
...
where
    <T as Config>::Currency: ReservableCurrency<
        T::AccountId,
        Balance = <<T as Config>::Currency as Inspect<AccountIdOf<T>>>::Balance,
    >,
{
...
pub mod test {
    ...
    use crate::{
        migrations::update_balance_for_public_credentials, mock::*, AccountIdOf, Config,
        CredentialIdOf, Credentials, Error, HoldReason,
    };
    ...
    fn test_setup() {
        ...
        let mut new_credential = generate_base_credential_entry::<Test>(
            ACCOUNT_00,
            0,
            attester.clone(),
            Some(ctype_hash_1),
        ...
        ExtBuilder::default()
            ...
            .build_and_execute_with_sanity_tests(|| {
                let hold_balance_pre_migration = <<Test as Config>::Currency as InspectHold<
                    AccountIdOf<Test>,
                >>::balance_on_hold(
                    &HoldReason::Deposit.into(), &ACCOUNT_00
                );

                assert_eq!(hold_balance_pre_migration, <Test as Config>::Deposit::get());

                kilt_support::migration::translate_holds_to_reserve::<Test>(
                    HoldReason::Deposit.into(),
                );
                ...
                let reserved_balance = <<Test as Config>::Currency as ReservableCurrency<
                    AccountIdOf<Test>,
    ...
    fn test_balance_migration_public_credential() {
        ...
        ExtBuilder::default()
            ...
            .build_and_execute_with_sanity_tests(|| {
                ...
                let public_credentials_pre_migration =
                    Credentials::<Test>::get(subject_id, credential_id);

                let reserved_pre_migration = <<Test as Config>::Currency as ReservableCurrency<
                    AccountIdOf<Test>,
                >>::reserved_balance(&ACCOUNT_00);
                ...
                let public_credentials_post_migration =
                    Credentials::<Test>::get(subject_id, credential_id);

                let reserved_post_migration = <<Test as Config>::Currency as ReservableCurrency<
                    AccountIdOf<Test>,
                >>::reserved_balance(&ACCOUNT_00);

                let balance_on_hold = <<Test as Config>::Currency as InspectHold<
                    AccountIdOf<Test>,
                >>::balance_on_hold(
                    &HoldReason::Deposit.into(), &ACCOUNT_00
                );
