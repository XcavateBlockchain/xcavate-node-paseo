...
use crate::{
    AttesterOf, BalanceOf, Config, CredentialEntryOf, CredentialIdOf, CredentialSubjects,
    Credentials, CtypeHashOf, InputClaimsContentOf, InputCredentialOf, InputSubjectIdOf,
    PublicCredentialDepositCollector, PublicCredentialsAccessControl,
};
...
pub fn generate_base_public_credential_creation_op<T: Config>(
    ...
    InputCredentialOf::<T> { ctype_hash, subject: subject_id, claims, authorization: None }
}
...
pub(crate) mod runtime {
    ...
    pub(crate) struct ExtBuilder {
        ...
        public_credentials:
            Vec<(<Test as Config>::SubjectId, CredentialIdOf<Test>, CredentialEntryOf<Test>)>,
    }
        ...
        pub fn build_and_execute_with_sanity_tests(self, test: impl FnOnce()) {
            self.build().execute_with(|| {
                ...
                crate::try_state::do_try_state::<Test>()
                    .expect("Sanity test for public credential failed.");
            })
        }
